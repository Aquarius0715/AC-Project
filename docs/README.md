...
版: 0.20.0 / 作成日: 2026-09-14 / 更新日: 2026-09-17 / 状態: レビュー用ドラフト / 言語: 日本語
...
現行baselineは[DOC-0.20.0](04-agentic-sdlc/runs/DOC-0.20.0/spec-manifest.json)。DOC-0.7.0〜0.19.0は旧版の記録であり、現在の実装入力として使用しない（DOC-0.18.0はbaseline未作成、DOC-0.19.0は独立G1で不合格、IR75）。検証は `python3 docs/tools/validate_documents.py` と `python3 docs/tools/check_review_regressions.py` の両方（IR73）。文書修正者による自己再レビューと、別主体によるG1承認は区別する。本番接続はD11の成果物確定までNOT READY。
