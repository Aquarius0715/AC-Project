...
一次資料: [企業要件原文（SRC-06）](../00-prepare/sources/company-requirements-original.txt)。原文を再整理した要件から画面・入力・状態・受入条件を設計する。入力: [役割別要件](../01-requirements/admin.md)、[共通要件](../01-requirements/common.md)。必読: [共通詳細設計](common.md)、[UIUX仕様書](../03-uiux/UIUXSpecification.md)。以下はフロントエンドの項目・表示・モック動作の設計。画面上の登録・割当・入金・制限・監査はすべて共有モックメモリの状態遷移で、サーバー実装やDB設計を依頼するものではない。
...
**一次資料との対応**: SRC-06 BIZ-04, BIZ-08 → FR-A01 → DD-A01。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 全体集計・担当導線。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 稼働率は最新状態を把握できる有効設備のうちpowerOnの割合とし、全設備数・不明数を必ず併記。顧客数はactiveの顧客組織数、請求遅延は未入金額ベース。
3. 閲覧のみ。ダッシュボードと遷移先一覧で期間・スコープ・定義が一致する。
4. 更新対象Query: `admin summary（関連イベント時）`。

**境界条件・失敗時**: 不明設備を稼働率の分母へ黙って含めない。異なる通貨を合算せず通貨別、データ0件の率は算定不可。

**検証**: 追跡表のAT-A01配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A02 詳細

**一次資料との対応**: SRC-06 BIZ-07 → FR-A02 → DD-A02。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 登録・編集・アーカイブ。フィールド型・必須性・初期値・操作順序は実装提案。

対象: FR-A02 / 主表示pattern: **UI-LIST / UI-FORM**。画面サービス境界は`organizations.list, organizations.save, customers.list, customers.save, properties.save, spaces.save, units.save, units.archive, units.list, units.get, properties.list, spaces.list, capabilities.list`。
...
| space.parentSpaceId / kind | ID・enum | 同物件・循環なし | 階層 |
...
2. 読取・操作に応じて次の業務条件を適用する: 新規設備はcustomerOrgId・spaceId・modelIdの整合必須。契約/案件/IoT紐付けがある設備は物理削除不可。別テナントへの移管は1A対象外。
3. 不変IDと版、変更前後・理由を保持。移設は現行の所属場所を更新し履歴に旧場所を残す。
4. 更新対象Query: `organizations / customers / properties / spaces / units / audit`。

**境界条件・失敗時**: 他顧客の部屋、循環階層、存在しないmodelId、使用中設備の削除を拒否。inactive顧客に新規設備を登録しない。

**検証**: 追跡表のAT-A02配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A03 詳細

**一次資料との対応**: SRC-06 BIZ-04 → FR-A03 → DD-A03。出所区分: 設計補完（企業目的に対応）。本節で具体化する設計補完: 権限・所属・担当期間の管理。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: roleと能力を分ける。管理者でもrestriction.manage/overrideを自動付与しない。外部技術者に終了なしの設備アクセスを付与しない。自分への高権限付与は別HQ権限管理者の操作に限定。
3. Membership版とscopeVersionを更新。旧セッションの閲覧キャッシュを破棄し、次変更要求は新権限で判定。
4. 更新対象Query: `members / session scope / all affected query caches / audit`。

**境界条件・失敗時**: 別テナントscope、外部で期限なし、validFrom>=validUntil、自分へのoverride追加を拒否。失効操作後の古い画面からの保存を拒否。

**検証**: 追跡表のAT-A03配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A04 詳細

**一次資料との対応**: SRC-06 BIZ-06, BIZ-20 → FR-A04 → DD-A04。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 機種能力・IoT台帳の編集。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 能力の未確認はfalse/unknown。対応modeを製品カテゴリから推定しない。既存automationが新能力に違反する場合は該当ルールを停止し理由を表示する。
3. Capability版を更新してUnit操作候補へ反映。未完了Commandの要求内容は履歴として保持し、勝手に新能力へ書換えない。
4. 更新対象Query: `capabilities / units / devices / automations / audit`。

**境界条件・失敗時**: min>max、step<=0、空mode集合でmode制御trueを拒否。非対応FW版を候補へ紛れ込ませない。

**検証**: 追跡表のAT-A04配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
検証: AT-A05-SRC — 窓開放の疑い・断熱不足の点検記録・根拠なしの3 fixtureで、文言・根拠・時刻が異なり、既読にしても異常が解消しない。

**一次資料との対応**: SRC-06 BIZ-08, BIZ-11, BIZ-17 → FR-A05 → DD-A05。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: しきい値の設定と異常処理。フィールド型・必須性・初期値・操作順序は実装提案。

対象: FR-A05 / 主表示pattern: **UI-LIST / UI-FORM**。画面サービス境界は`alerts.list, policies.save, notifications.preview, policies.list, policies.get`。
...
2. 読取・操作に応じて次の業務条件を適用する: 単位はmetricに従い変更不可。missing/staleは測定閾値の正常判定に使わず通信/品質通知へ。繰返し通知はcooldownで抑制し、重大度変化は新しい通知理由として保持する。
3. Policy版を保存。発火でAlertとNotificationプレビューを作成。通知既読とAlert確認を分離。
4. 更新対象Query: `policies / alerts / notifications / admin summary / audit`。

**境界条件・失敗時**: 通知先0件、継続時間0、解除閾値と比較方向の不整合を拒否。閾値到達直前/一致/継続時間境界を検証する。

**検証**: 追跡表のAT-A05配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
2. 読取・操作に応じて次の業務条件を適用する: 社内はHQが担当・日程を確定、外注はoffer受諾後に自社割当。定期計画の繰返し設定は次回生成予定を表示し、同じ計画/回の案件は1件。
3. Job/Assignment/Offer/費用明細・品質履歴を同じjobIdに集約。完了してもAlert解消は別判断。
4. 更新対象Query: `jobs / plans / offers / assignments / costs / notifications / audit`。

**境界条件・失敗時**: 辞退後の再委託、確定予定重複、期限超過、品質差戻しを検証。見積通貨と実績通貨が違う場合は変換せず別集計。

**検証**: 追跡表のAT-A06配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
2. 読取・操作に応じて次の業務条件を適用する: 一般保守はrestrictionEligible=false。RTOでも明示的に制限可能とした契約だけ対象。既に発行した請求は契約改版で遡及変更しない。
3. 契約版を保持し、新請求には新しい版、過去請求には元の版を参照させる。契約終了を実機停止として扱わない。
4. 更新対象Query: `contracts / customer payments / audit`。

**境界条件・失敗時**: 他顧客設備、期間逆転、負料金、一般保守の制限有効化を拒否。契約なし設備の監視・保守を妨げない。

**検証**: 追跡表のAT-A07配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
notifications.previewで請求・案内チャネル・選択済み方法を確認する。HQは既存Paymentの入金確認をpayments.confirm、Paymentがない請求の手動入金をpayments.recordManualで行い、カード選択フォームやpayments.simulateによる顧客の決済操作は配置しない。案内閲覧・プレビューだけではpaidへ遷移しない。
...
対象: FR-A08 / 主表示pattern: **UI-LIST / UI-FORM**。画面サービス境界は`invoices.list, invoices.create, payments.confirm, notifications.preview, inquiries.list, inquiries.answer, payments.recordManual, contracts.list`。
...
2. 読取・操作に応じて次の業務条件を適用する: 1Aは全額入金のみ。invoiceId+paymentReferenceの二重確認は同じ結果を返す。督促対象は未入金・期限超過で、例外や係争の有無を表示する。
3. Payment confirmed、Invoice paid、監査を記録。関連RestrictionのcauseInvoiceIds全件がpaidなら、scheduledはcancelled、requested/appliedはrelease_requestedへ進める。未入金が残れば解除せず、機器応答待ちと残件数を表示する。
4. 更新対象Query: `invoices / payments / restrictions / notifications / audit`。

**境界条件・失敗時**: 金額違い、通貨違い、同参照の別請求流用、paidの再請求を拒否。入金済みになった直後の督促実行を再確認して止める。

**検証**: 追跡表のAT-A08配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A09 詳細

**一次資料との対応**: SRC-06 BIZ-21 → FR-A09 → DD-A09。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 予告・実行確認・機器応答。フィールド型・必須性・初期値・操作順序は実装提案。
...
| フィールド | 型・必須性 | 初期値・制約 | 用途 |
|---|---|---|---|
| contractId / causeInvoiceIds / unitIds | 必須 | 同契約の全期限超過未入金請求を原因集合として固定。設備ごとに進行中制限は1件 | 対象 |
...
2. 読取・操作に応じて次の業務条件を適用する: UIで期限到来しても自動実停止しない。1AはHQの明示確認で模擬要求。対象全台が応答するまでapplied/releasedにしない。停止と温度制限は別policy。
3. Restrictionと設備別Commandを作成。原因請求causeInvoiceIds全件の入金確認後はscheduledならcancelled、requested/appliedならrelease_requested。1件でも未入金なら状態を維持する。
4. 更新対象Query: `restrictions / commands / units / customer billing / notifications / audit`。

**境界条件・失敗時**: 実行直前に原因請求全件が入金済みならcancelledとし適用要求を作らない。猶予・例外中、未通知、非対応機器も適用不可。一部offlineでは全体をappliedにせず設備別保留を表示。解除失敗ではrelease_requestedを保持し、遅い適用応答でappliedへ戻さない。

**検証**: 追跡表のAT-A09配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A10 詳細

**一次資料との対応**: SRC-06 BIZ-21 → FR-A10 → DD-A10。出所区分: 設計補完（企業目的に対応）。本節で具体化する設計補完: 猶予・例外・監査手順。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 適用前の取消はcancelled。適用要求後は反映が不明でも解除フローへ。例外・猶予の期限が切れても自動再適用せず条件再確認を要する。
3. 例外・猶予の変更前後と期限、解除の理由・主体を記録。手動解除でInvoiceの未入金を解消しない。
4. 更新対象Query: `restrictions / commands / audit / notifications`。

**境界条件・失敗時**: overrideなしHQ、空理由、過去の猶予期限を拒否。requested中の取消でも実際の適用可能性をゼロと推定しない。

**検証**: 追跡表のAT-A10配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
| action | UnitAction/必須 | 能力・制限内 | 動作 |
...
**検証**: 追跡表のAT-A11配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
検証: AT-A12-SRC — 未計測・非対応・合成観測のfixtureで表示を切り替え、未計測で0や「安全」を表示しない。数値の単位欠落は不明になる。
...
**検証**: 追跡表のAT-A12配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A13 詳細

**一次資料との対応**: SRC-06 BIZ-23, BIZ-25 → FR-A13 → DD-A13。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 基準版・算定条件の管理。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 基準は設備集合・境界・期間条件・モデル版を保持。気象等の補正モデル未実装なら補正済みと表示しない。負の削減を0へ丸めない。
3. 基準保存時に新しい版。既存MRVレポートの基準版を遡及変更しない。
4. 更新対象Query: `baselines / energy / audit`。

**境界条件・失敗時**: 基準0、実績欠測、異なる設備集合、異なる算定境界を検証。100/80なら20kWh・20%、100/120なら-20kWh・-20%。

**検証**: 追跡表のAT-A13配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A14 詳細

**Scope 2報告プレビューの表示・処理設計（BIZ-25）**

レポート画面モデルにreportCategory=scope2_electricity、organizationId、period、siteIds、gridRegion、factorValue、factorUnit、factorYear、factorVersion、boundaryDescription、coverageRatioを追加。これは電力利用に関するデモ集計の分類。係数・単位・地域・対象境界が不足する場合は算定未完了とし、0で補完しない。省エネ量と電力由来排出量を別欄にする。画面と書き出しに「デモ・未検証」を残す。

検証: AT-A14-SRC — 同じ使用量で係数版を変更すると換算結果と版が変わる。係数欠落で算定未完了を表示し、期間外・対象外拠点を混入しない。

**一次資料との対応**: SRC-06 BIZ-25 → FR-A14 → DD-A14。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: レポート項目・証跡・プレビュー。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 係数の地域・年度・単位・出典を必須。確認履歴はdemo_reviewedで外部認証と区別。入力版が変われば新しい報告版を作り、旧結果は保持する。
3. MRVReportに係数/基準のスナップショット参照と算定結果・品質・reviewHistoryを保存。プレビューのみでは確定記録を作らない。
4. 更新対象Query: `mrv / review history / audit`。

**境界条件・失敗時**: 係数欠落、単位不整合、coverage0、同一報告版への重複確認を検証。未検証値が「認証済み」にならない。

**検証**: 追跡表のAT-A14配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
...
検証: AT-A15-SRC — オフセット未選択で申込が作られない。市場構想を開いても残高・実証明・取引結果が生成されず、模擬償却とは異なる状態を表示する。

**一次資料との対応**: SRC-06 BIZ-24, BIZ-26 → FR-A15 → DD-A15。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 模擬償却・市場構想のプレビュー。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 購入申込と購入確認と償却を別イベントにする。1Aは1記録の全量償却のみ。自社排出削減計算を購入済み残高に加算しない。
3. quoted→demo_requested→demo_purchased→demo_retiredを履歴に保存。証明参照はDEMO-接頭辞で実証明として出力しない。
4. 更新対象Query: `offsets / offset events / audit`。

**境界条件・失敗時**: 未購入の償却、二重償却、希望量0、見積失効、別テナントの記録操作を拒否。失敗はfailedと直前段階を保持。

**検証**: 追跡表のAT-A15配下（N/E/B・該当SRC/R01）と該当Sシナリオ。

### DD-A16 詳細

**一次資料との対応**: SRC-06 BIZ-20 → FR-A16 → DD-A16。出所区分: 企業原文 SRC-06＋設計補完。本節で具体化する設計補完: 監査フィルター・相関ID。フィールド型・必須性・初期値・操作順序は実装提案。
...
2. 読取・操作に応じて次の業務条件を適用する: 監査は成功/拒否/失敗を分離し、before/afterは秘密・連絡先をマスク。フロント上は追加のみで編集/削除を提供しない。ブラウザ内デモに耐改ざん保証はしない。
3. 閲覧のみ。条件をURLへ保持するが機密本文をURLへ入れない。
4. 更新対象Query: `なし（監査参照）`。

**境界条件・失敗時**: 他テナント相関ID、削除API相当の呼出し、期間逆転を拒否。実行主体が役割切替後の別人に書換わらない。

**検証**: 追跡表のAT-A16配下（N/E/B・該当SRC/R01）と該当Sシナリオ。
