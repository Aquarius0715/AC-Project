...
入力: [役割別要件](../01-requirements/admin.md)、[共通要件](../01-requirements/common.md)。必読: [共通詳細設計](common.md)、[UIUX仕様書](../03-uiux/UIUXSpecification.md)。以下はPROPOSEDのフロントエンド契約であり、稼働中のAPI仕様ではない。
...
対象: FR-A01 / 主表示pattern: **UI-OVERVIEW**。API境界は`admin.summary`。
...
4. 更新対象Query: `admin summary（関連イベント時）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 不明設備を稼働率の分母へ黙って含めない。異なる通貨を合算せず通貨別、データ0件の率は算定不可。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A02 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`organizations.list, organizations.save, customers.list, customers.save, properties.save, spaces.save, units.save, units.archive, units.list, units.get`。
...
4. 更新対象Query: `organizations / customers / properties / spaces / units / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 他顧客の部屋、循環階層、存在しないmodelId、使用中設備の削除を拒否。inactive顧客に新規設備を登録しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A03 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`members.list, members.save`。
...
4. 更新対象Query: `members / session scope / all affected query caches / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 別テナントscope、外部で期限なし、validFrom>=validUntil、自分へのoverride追加を拒否。失効操作後の古い画面からの保存を拒否。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A04 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`capabilities.list, capabilities.save, devices.list`。
...
4. 更新対象Query: `capabilities / units / devices / automations / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: min>max、step<=0、空mode集合でmode制御trueを拒否。非対応FW版を候補へ紛れ込ませない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A05 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`alerts.list, policies.save, notifications.preview`。
...
4. 更新対象Query: `policies / alerts / notifications / admin summary / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 通知先0件、継続時間0、解除閾値と比較方向の不整合を拒否。閾値到達直前/一致/継続時間境界を検証する。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A06 / 主表示pattern: **UI-LIST / UI-DETAIL / UI-FORM**。API境界は`jobs.list, jobs.create, jobs.offer, jobs.assign, jobs.review, jobs.saveCost, jobs.hold, jobs.resume, jobs.cancel, plans.save, plans.generateNext, jobs.get`。
...
4. 更新対象Query: `jobs / plans / offers / assignments / costs / notifications / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 辞退後の再委託、確定予定重複、期限超過、品質差戻しを検証。見積通貨と実績通貨が違う場合は変換せず別集計。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A07 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`contracts.list, contracts.save`。
...
4. 更新対象Query: `contracts / customer payments / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 他顧客設備、期間逆転、負料金、一般保守の制限有効化を拒否。契約なし設備の監視・保守を妨げない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A08 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`invoices.list, invoices.create, payments.confirm, notifications.preview, inquiries.list, inquiries.answer`。
...
4. 更新対象Query: `invoices / payments / restrictions / notifications / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 金額違い、通貨違い、同参照の別請求流用、paidの再請求を拒否。入金済みになった直後の督促実行を再確認して止める。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A09 / 主表示pattern: **UI-LIST / UI-DETAIL / UI-FORM**。API境界は`restrictions.schedule, restrictions.execute, restrictions.release, commands.get, restrictions.list, restrictions.get`。
...
4. 更新対象Query: `restrictions / commands / units / customer billing / notifications / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 開始直前の入金、猶予、例外、未通知、非対応機器を再検証。一部offline、遅い適用応答、解除失敗を表示し状態を逆行させない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A10 / 主表示pattern: **UI-DETAIL / UI-FORM / UI-TIMELINE**。API境界は`restrictions.defer, restrictions.exempt, restrictions.cancel, restrictions.override, audit.list, restrictions.get`。
...
4. 更新対象Query: `restrictions / commands / audit / notifications`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: overrideなしHQ、空理由、過去の猶予期限を拒否。requested中の取消でも実際の適用可能性をゼロと推定しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A11 / 主表示pattern: **UI-FORM**。API境界は`policies.save, automations.simulate`。
...
4. 更新対象Query: `policies / automations / simulation results / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 顧客ルールよりHQ方針を優先、同順位の結果が毎回同じ、太陽光データ欠測で勝手に実行しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A12 / 主表示pattern: **UI-FORM / UI-ANALYSIS**。API境界は`policies.save, automations.simulate`。
...
4. 更新対象Query: `policies / alerts / commands / notifications / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: ppmとµg/m³の閾値を混用しない。未計測は正常/回復判定に使わない。換気非対応機器へ送風コマンドを代用しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A13 / 主表示pattern: **UI-ANALYSIS / UI-FORM**。API境界は`energy.summary, baselines.list, baselines.save`。
...
4. 更新対象Query: `baselines / energy / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 基準0、実績欠測、異なる設備集合、異なる算定境界を検証。100/80なら20kWh・20%、100/120なら-20kWh・-20%。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A14 / 主表示pattern: **UI-ANALYSIS / UI-FORM / UI-DETAIL**。API境界は`mrv.preview, mrv.saveDraft, mrv.recordReview, factors.list, factors.save, mrv.list, mrv.get`。
...
4. 更新対象Query: `mrv / review history / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 係数欠落、単位不整合、coverage0、同一報告版への重複確認を検証。未検証値が「認証済み」にならない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A15 / 主表示pattern: **UI-LIST / UI-FORM / UI-DETAIL**。API境界は`offsets.preview, offsets.simulate, offsets.list`。
...
4. 更新対象Query: `offsets / offset events / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 未購入の償却、二重償却、希望量0、見積失効、別テナントの記録操作を拒否。失敗はfailedと直前段階を保持。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-A16 / 主表示pattern: **UI-TIMELINE / UI-DETAIL**。API境界は`audit.list, devices.events`。
...
4. 更新対象Query: `なし（監査参照）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 他テナント相関ID、削除API相当の呼出し、期間逆転を拒否。実行主体が役割切替後の別人に書換わらない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
