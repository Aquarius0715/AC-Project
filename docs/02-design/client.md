...
入力: [役割別要件](../01-requirements/client.md)、[共通要件](../01-requirements/common.md)。必読: [共通詳細設計](common.md)、[UIUX仕様書](../03-uiux/UIUXSpecification.md)。以下はPROPOSEDのフロントエンド契約であり、稼働中のAPI仕様ではない。
...
対象: FR-C01 / 主表示pattern: **UI-OVERVIEW**。API境界は`units.list, telemetry.summary, alerts.list`。
...
4. 更新対象Query: `units / telemetry / alerts（購読イベント時のみ）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 1台だけ未計測にした場合、正常台数へ加えず不明台数に計上する。別顧客の設備を検索条件へ指定しても集計しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C02 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`properties.list, properties.save, properties.archive, spaces.list, spaces.save, spaces.archive, units.list`。
...
4. 更新対象Query: `properties / spaces / units / customer summary`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 空の名称、121文字、循環、他組織の親IDを拒否。409時は現在版を提示して入力を保持し、自動上書きしない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C03 / 主表示pattern: **UI-DETAIL**。API境界は`units.get, commands.create, commands.get`。
...
4. 更新対象Query: `commands / unit detail / telemetry summary / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 16〜30°C/1°C刻みのデモ能力なら15、31、24.5を拒否。offline、他顧客、制限違反、遅延応答でも成功扱いしない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C04 / 主表示pattern: **UI-LIST / UI-FORM**。API境界は`automations.list, automations.save, automations.simulate`。
...
4. 更新対象Query: `automations / next-run preview / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 曜日0件、終了動作なし、曖昧/存在しない夏時間のローカル時刻を拒否。停止後の予約イベントでは要求を作らない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C05 / 主表示pattern: **UI-FORM**。API境界は`consents.get, consents.update, automations.save, automations.simulate`。
...
4. 更新対象Query: `consents / automations / simulation results / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 同意なしでlocation条件を有効化できない。取消後の帰宅イベントはコマンド0件。天候データ欠測ではスキップ理由を表示する。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C06 / 主表示pattern: **UI-ANALYSIS**。API境界は`energy.summary, baselines.list`。
...
4. 更新対象Query: `energy（検索条件変更時のみ）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 基準100/実績80kWh/単価0.5MYRなら節約10MYR。基準0は率なし、実績120は増加20%、欠測はcoverage付き。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C07 / 主表示pattern: **UI-ANALYSIS**。API境界は`telemetry.series, units.get, commands.create`。
...
4. 更新対象Query: `telemetry / commands（換気時） / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: CO₂欠測でもPM2.5値は表示可能。fan能力だけの設備でventilationコマンドを生成しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C08 / 主表示pattern: **UI-LIST**。API境界は`alerts.list, notifications.markRead, notifications.list`。
...
4. 更新対象Query: `notifications / unread count`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 取得失敗時に異常0件表示をしない。通知先の設備が失効した場合は詳細を漏らさず利用不可を表示。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C09 / 主表示pattern: **UI-LIST / UI-FORM / UI-DETAIL**。API境界は`jobs.list, jobs.create, jobs.get, jobs.cancel, jobs.addNote`。
...
4. 更新対象Query: `jobs / job events / admin summary / notifications`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 症状9文字/2001文字、過去の希望枠、設備なしを拒否。通信失敗で症状を保持。二重送信で案件が増えない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C10 / 主表示pattern: **UI-LIST / UI-DETAIL**。API境界は`contracts.list, invoices.list`。
...
4. 更新対象Query: `contracts / invoices（閲覧のみ）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 顧客bのinvoiceIdを顧客aが指定しても取得しない。paid請求に支払い開始ボタンを表示しない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C11 / 主表示pattern: **UI-DETAIL / UI-FORM**。API境界は`invoices.get, payments.simulate, notifications.preview`。
...
4. 更新対象Query: `payments / invoices / restrictions / notifications / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 二重クリック/同一参照の確認で二重計上しない。決済失敗で元の未入金状態と支払い再試行導線を維持する。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C12 / 主表示pattern: **UI-DETAIL**。API境界は`restrictions.forInvoice, commands.get, inquiries.create`。
...
4. 更新対象Query: `inquiries / inquiry events（送信時のみ）`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: offline設備が1台残れば全体を解除済みとしない。顧客はURL/サービス呼出しでもoverrideできない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
...
対象: FR-C13 / 主表示pattern: **UI-ANALYSIS / UI-FORM**。API境界は`energy.summary, offsets.preview, offsets.simulate, offsets.list`。
...
4. 更新対象Query: `offsets / audit`。読取だけのケースは業務mutationを発行しない。複数更新はRepository内の1コマンドでまとめる。

**競合・失敗時**: 希望量0/負値、見積失効、別顧客の算定結果を拒否。模擬償却と外部認証を同一文言にしない。 共通HTTP/DomainErrorは[実装契約](implementation-contracts.md)の表示・再試行表へ変換する。409後の再送は更新された対象を利用者が確認してから行う。
