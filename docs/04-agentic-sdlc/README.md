...
現行入力は[runs/DOC-0.17.0/spec-manifest.json](runs/DOC-0.17.0/spec-manifest.json)の全ファイル。静的検証は `python3 docs/tools/validate_documents.py`。仕様が変わったらmanifestを再生成し、旧レビュー証跡を流用しない。自己レビューは独立G1の代用ではない。別主体のレビュー記録がない場合gate-G1はpendingを維持し、実装完了/試験合格を記録しない。
