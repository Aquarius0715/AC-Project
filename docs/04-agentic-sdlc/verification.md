...
この版では、4つの役割にある49個の要件それぞれに、AT-番号-N(正常系)、-E(境界・例外)、-B(業務条件)という3種類の受入条件を定義し、合計147個のケースに分けた。企業原文を補うAT-*-SRCの10件、追加のR01の10件、共通の7件・非機能の8件と合わせると、要件に対するケースのまとまりは合計182件になる(役割別のN/E/Bが147、原文補完のSRCが10、追加のR01が10、共通が7、非機能が8)。UI専用のケースと、役割をまたいだSシナリオは、この件数とは別に検証する。182という数字は管理のために数えたケースのまとまりの数であり、1つ1つの確認項目(assertion)の数ではない。各受入行にある①②…という番号は、記載されている順番のsubcase(例: AT-C06-E.01)を表し、Given側(前提)とThen側(期待結果)で同じ番号のものが、入力と期待値のペアになる。テスト担当のエージェントは、このペアをそのまま試験に書き写すこと。実装の中身を見て期待値を補ってはいけない。境界値のそれぞれの変化パターンは、別々のsubcaseとして扱う。manifest(仕様ファイルの一覧)は、仕様のbaseline(基準)と合わせてG1の段階で確認する。設計担当のエージェントが期待値の不足を修正するまで、該当のケースはready(着手可能)にしない。すべてのsubcaseと該当するSRC/R01を満たすまで、親となるATを合格(passed)にしない。
...
| 手動・自動アクセシビリティ | キーボード操作、スクリーンリーダー、コントラスト比、360/768/1280px幅、200%拡大、日本語・英語 | 検証した環境と実際の操作・結果。自動チェックだけで基準を満たしたと宣言しない |
...
期待する結果: 期限より前の実行は拒否される。制限の適用や解除は、対象のすべての機器に反映されるまでは完了扱いにしない。実際のカード番号の入力や外部への送信は発生しない。決済処理中は未入金の状態のままで、確認が済んで初めてpaidになる。オフラインの設備は保留状態になる。監査ログには、操作した人、理由、時刻、対象、相関ID(関連する操作をつなぐID)が残る。
...
前提: 英語・日本語の辞書データ、音声はシミュレーション(模擬)。操作: 言語を切り替える→主要な画面・通知・AIの応答も切り替わる→音声デモで温度を確認する→設定変更の内容を確認する→マイクが拒否された場合、または未対応の場合はテキストで続ける。
...
| ケース | 追加で確認すること |
|---|---|
| AT-X01 / AT-X02 | パスワード再設定時に一般的な文言を使うこと、サインアウト時に古い表示が消えること、英語・日本語、音声を拒否した場合の代替手段 |
| AT-X03 / AT-X06 | 4つの役割すべてで、データの品質・単位・能力の表示が一致すること、RTO(遠隔操作対応)でない設備を正常なものとして扱うこと |
...
| AT-NFR08 | 保存時のUTC(協定世界時)、表示するタイムゾーン、日本語・英語、長い翻訳文、金額・単位の書式 |
...
| AT-UX-REF02 | 英数字の本文にPlus Jakarta Sansというフォントが使われ、IDの表示にGeist Monoが使われ、日本語には代替フォントが使われること。h1の見出しにBricolageというフォントを自動で適用しないこと |
