# Correction Report for Independent G1 Findings DOC-0.20.0 (2026-09-17)

Corrected baseline: `4f45da02ef91f0cf93075848d6ded3373878521740aa2f4047673eee709e4acc` ([spec-manifest](spec-manifest.json)).

Scope: findings G1-001–031 from the independent G1 review of DOC-0.19.0 (branch docs/0.19.0-independent-review, commit 0505446) by another agent ([decision record](../DOC-0.19.0/independent-g1/review.md), FAIL, 12 MAJOR, 17 MINOR, 2 QUESTION). The AI in this conversation (Opus 5) made the corrections and is separate from the G1 reviewer. The post-correction check is a **self-review** by the correction author, not an independent G1 decision for DOC-0.20.0 ([gate-G1.yaml](gate-G1.yaml)=pending; request another agent). App implementation and runtime tests are not_run. Production HTTP/authentication/DB/physical devices are outside 1A under DEC-12 and D11.

## 1. Executive Review Summary

Before correction (independent G1 for DOC-0.19.0): **NOT READY (0 BLOCKER, 0 CRITICAL, 12 MAJOR, 17 MINOR, 2 QUESTION)**.

Post-correction self-review: all 31 findings have corrections or answers (29 resolved, 2 answered). All 64 requirements in the traceability matrix are OK (before correction: 9 CONFLICT, 32 INCOMPLETE, 23 OK). Both `validate_documents.py` (including TypeScript strict checks with `--tsc`) and `check_review_regressions.py` (all 84 mutations detected) passed. New design proposals DEC-54–59 are PROPOSED (reversible), for owner review before company acceptance. Independent G1 reassessment has not run (pending).

Main corrections:

1. **Technician write authorization (G1-001)**: IR94 defines authorization qualifiers and adds a table that checks both internal and external technicians against their own active Assignment and work window. Assigned jobs were added to acceptance fixtures.
2. **Business event notifications (G1-002)**: The IR95 table makes templateKey, recipients, and counts deterministic.
3. **Restriction cancellation (G1-003, G1-013)**: Added the IR96 state table and releaseIntent to the canonical type.
4. **Acceptance input data (G1-004, 005, 009, 010, 011, 012, 019)**: IR97 defines in-range readings, synchronization of Assignment and job slots, complete saved input objects, and creation of states through operations. The validator now checks all patches. Old acceptance expectations were also aligned with current rules.
5. **Air quality, allergens, and inspection scope (G1-006, 007, 008, 027)**: IR98–IR100 define seed data, thresholds, component sets, and submission validation.
6. **Common acceptance (G1-025)**: Added concrete N/E/B value tables and fixtures for AT-X01–X07. DEC-59 defines the required failure behavior.

## 2. BLOCKER Issues

None (also 0 in independent G1).
...
For each item, Problem through Suggested Revision quote the independent G1 review. Applied Revision describes the corrections.
...
- Location: operation-catalog.csv:3 (alerts.acknowledge), :20 (commands.create), :35 (devices.addResponseNote), :37 (devices.calibrate), :45 (devices.updateFirmware) / strict-review-contracts.md:24 / review-resolution-contracts.md:343, 345, 430 / implementation-contracts.md:240, 288 / deterministic-contracts.md:21, 25, 83 / technician.md:201 (AT-T07-N), :284 (AT-T11-E③), :308 (AT-T12-N) / acceptance-review-019.csv:22 (AT-REV19-021) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Defined authorization qualifiers in a table and added a table that checks technician writes (internal and external) against their own active Assignment and work window. An internal technician without an assignment gets FORBIDDEN (errors.assignment_required) for operations whose input type has no jobId; an explicit jobId without an assignment gets IR93 NOT_FOUND. updateFirmware returns OFFLINE if the device is not online at request time, without a DeviceOperation. Added assigned job job-t07 (shared:tech-internal-a-job-online) to AT-T07-N/AT-T07-SRC and job-t12 to AT-T12-N. Updated AT-T11-E③ and AT-REV19-021. Catalog notes now refer to IR94.
- Contract: [IR94](../../../02-design/review-resolution-contracts.md#ir94-authorization-column-qualifiers-and-technician-write-conditions--g1-001--g1-026--g1-030), DEC-54 (PROPOSED, reversible)
- Acceptance: AT-G120-001 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Added a per-event table of templateKey, target, and recipients, plus common rules (inApp, simulated, one per recipient, exclude actor, recheck, and no increase for the same event). Added job_update and device_operation to canonical templateKey/NotificationType. Delegated the notification table in implementation-contracts.md to IR95. AT-P03-N④ now gives exact counts (4 notifications, 0 for the actor).
- Contract: [IR95](../../../02-design/review-resolution-contracts.md#ir95-business-event-notifications--g1-002), DEC-55 (PROPOSED, reversible)
- Acceptance: AT-G120-002 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: review-resolution-contracts.md:236, 238, 240 / Design admin.md:55, 395 / 02-design/common.md:181, 185 / Requirements admin.md:268 (BR-A10), :275 (AT-A10-E④), :276 (AT-A10-B②) / strict-review-contracts.md:127 (SR19) / service-contracts.ts:84, 281 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added a cancellation state table: scheduled→cancelled; requested/applied→release_requested with source=cancel; release_requested is idempotent; released/cancelled return CONFLICT. Added cancellation to the IR35 trigger paths and aligned DD-A10, common §5, and AT-A10-B/E④.
- Contract: [IR96](../../../02-design/review-resolution-contracts.md#ir96-restriction-cancellation--g1-003-and-g1-013), DEC-56 (PROPOSED, reversible)
- Acceptance: AT-G120-003 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: AT-C06-E.3 now uses two Units × 60 kW with baseline 100 kWh, both within D07 limits. The validator checks readings and valid ranges in all acceptancePatches. Updated references in AT-A13-B②, AT-REV18-024, and AT-REV19-006/041.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031)
- Acceptance: AT-G120-004 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: acceptancePatches["AT-A12-N"] adds sensor-tamper-co2 to device-tamper. Under the same key, defined Policy input (co2 gte 1000, recoveryThreshold 900, durationSeconds 60, etc.) and evaluation input for both Units at 1100 ppm with observedAt=00:59:00Z. Made AT-A12-N/B results, Commands, and notifications explicit.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031)
- Acceptance: AT-G120-005 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Added available unit-online-rto and unsupported unit-non-rto observations to demoSeed.allergenObservations, plus selection rules and DemoTrigger allergen/load_alert. Added four fixtures for AT-C07-SRC/A12-SRC (fourth keys end in .4).
- Contract: [IR98](../../../02-design/review-resolution-contracts.md#ir98-demo-data-for-allergen-observations-and-possible-cause-alerts--g1-006-and-g1-027), DEC-57 (PROPOSED, reversible)
- Acceptance: AT-G120-006 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements client.md:195 (Basic flow), :196 (BR-C07), :202 (AT-C07-N②“Guidance: Ventilation recommended”) / Design client.md:272-277 / deterministic-contracts.md:119 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added a guidance table: co2≥1000 ppm means ventilation/manual control; pm25≥35 µg/m³ means cleaning/inspection; otherwise none, or insufficient data. Updated DD-C07 and AT-C07-N/E/B, including B④–⑦.
- Contract: [IR99](../../../02-design/review-resolution-contracts.md#ir99-air-quality-guidance-display--g1-007), DEC-57 (PROPOSED, reversible)
- Acceptance: AT-G120-007 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: UnitDetail.components now includes all serviceScope groups with 8/5/5 components, plus eight submission validation rules. Added shared:report-draft-all-normal with all 18 components marked normal for AT-T04/T05/T06/T08/T09-N, AT-C09-N, and AT-P01-N. Updated DD-T02/T09 field tables.
- Contract: [IR100](../../../02-design/review-resolution-contracts.md#ir100-inspection-component-sets-and-submission-validation--g1-008), DEC-58 (PROPOSED, reversible)
- Acceptance: AT-G120-008 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Defined complete input objects, including enabled=true, plus trigger/evaluation data in acceptancePatches for AT-A05-N/A11-N/A12-N, and referenced them in the prose. “Notification/preview” means stored inApp simulated Notification under IR97 §6.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031)
- Acceptance: AT-G120-009 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements client.md:140 (AT-C04-E③) / acceptance-fixes.csv:20 (AT-FIX-019) / fixture-contract.json:14-15 (customer-a validFrom=2026-09-01, validUntil=2026-10-01; same for all actors), :371 (note) / review-resolution-contracts.md:246 (IR36“Membership.validUntil expires normally on clock jumps”), :678 (IR92 rule 1) / deterministic-contracts.md:18-21, 141 / verification.md:42 (`validFrom <= now < validUntil`) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AT-C04-E③ and AT-FIX-019 keep the seed clock within Membership validity and save Sunday 02:30 on 2027-03-14 and Sunday 01:30 on 2026-11-01. D09 validation covers 366 days and returns VALIDATION at D01 priority 7.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-010 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Given patches for AT-REV19-002/015/037 set Assignment scheduled*/valid* and Job.scheduledSlot to the same values. jobs.start uses the normal operation. Case 037 checks the display at 02:00Z before reassignment. The validator checks synchronization in all patches under IR97 §2.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031)
- Acceptance: AT-G120-011 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Fixed AT-REV17-004 dueAt to 04:00Z; case 005 uses a dependency-free unit-archive-demo patch under the same key; case 014 keeps the known route URL and shows not-found. AT-REV18-003 expects restored(power), then tamper. Added acceptance plan CSVs to stale-text and patch-key checks.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-012 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: service-contracts.ts:84 (Restriction), :86 (RestrictionReleaseView) / review-resolution-contracts.md:26, 236 / operation-catalog.csv authorization column for restrictions.reconcile/retry (`release-intent-or-terminal-recovery-only`) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added releaseIntent with source/at/actorMembershipId to canonical Restriction, and releaseIntent {source,at} to RestrictionReleaseView. Defined reconcile/retry visibility for users who only have override permission.
- Contract: [IR96](../../../02-design/review-resolution-contracts.md#ir96-restriction-cancellation--g1-003-and-g1-013), DEC-56 (PROPOSED, reversible)
- Acceptance: AT-G120-013 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: screen-catalog.csv:8 (SCR-C08 operations=alerts.list;notifications.markRead;notifications.list) / review-resolution-contracts.md:355 / Design client.md:298-303 / Requirements client.md:229 (AT-C08-B①–③) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added summaries.get for alertCount to SCR-C08 and the same Page api_dependency, DD-C08 service boundary, and fields.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-014 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements client.md:310 (AT-C12-E①“offline units are pending”) / service-contracts.ts:83 (releaseState enum) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AT-C12-E① defines pending as releaseState=requested for less than 30 seconds after the remove Command is created, then failed at or after 30 seconds (the aggregate stays release_requested).
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-015 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: UX-05 gives an overview of received information. Canonical props are in component-contracts.csv, at IR72 priority 5.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-016 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: screen-catalog.csv:15 (SCR-P03 url_selection=tab,sort) / Design contractor.md:122 (jobId required) / deterministic-contracts.md:145 / strict-review-contracts.md:80 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added jobId to SCR-P03 url_selection.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-017 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: deterministic-contracts.md:93 / review-resolution-contracts.md:345, 631 / acceptance-review-019.csv:38 (AT-REV19-037) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: D06 confirmed-overlap checks exclude the active Assignment being replaced for the same jobId.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-018 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: review-resolution-contracts.md:681 (IR92 rule 4“patch only the relevant fields”), :669 (IR91 rule 10), :195 (IR31) / Requirements admin.md:276 (AT-A10-B①) / Requirements client.md:311 (AT-C12-B) / fixture-contract.json `acceptancePatches["AT-P01-N"]` (change only job-contractor-a status=submitted) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: IR92 rule 4 requires normal operations for Restriction.state, Job.status tied to a report version, and Payment/Invoice states, rather than patches. AT-A10-B, C12-B, and P01-N now give operation steps. Removed the job-contractor-a status patch from P01-N. The validator rejects state-only changes.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031)
- Acceptance: AT-G120-019 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements admin.md:362 (BR-A14“Factor requires region, year, unit, and source”), :369 (AT-A14-E“Factor unit does not match”) / service-contracts.ts:112 (EmissionFactor has no unit field) / review-resolution-contracts.md:617 (factorUnit has fixed display) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: The factor unit is fixed at kgCO₂e/kWh under IR88. BR-A14 and DD-A14 require region/year/source. Removed unit-mismatch cases from AT-A14-E/B.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-020 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements contractor.md:161 (AT-P05-B③) / Design technician.md:148, 289 / service-contracts.ts:70 (ReviewAvailability reasons) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AT-P05-B③: submitting not_inspected without a reason returns VALIDATION and does not add an item to quality review.
- Contract: [IR100](../../../02-design/review-resolution-contracts.md#ir100-inspection-component-sets-and-submission-validation--g1-008), DEC-58 (PROPOSED, reversible)
- Acceptance: AT-G120-021 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements client.md:152 (BR-C05“Consent for general use”), :160 (AT-C05-B①“General consent only”) / service-contracts.ts:99 (Consent.purpose only allows `location_automation`) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Only purpose=location_automation is supported. Removed general-use consent from BR-C05 and DD-C05. AT-C05-B① sets location consent granted=false.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-022 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements technician.md:309 (AT-T12-E②“Old heartbeat after new communication loss”) / service-contracts.ts:142 (DemoTrigger device kind: communication_lost/power_lost/tamper/restored) / strict-review-contracts.md:133 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AT-T12-E② expects communication_lost seq 5, then restored (axis=connection, seq 4).
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-023 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Applied Revision: Made inputs and steps explicit: AT-C13-N purpose, period, and unitIds; C09-N assignment slot, clock, and report input; A15-N input for each offsets.simulate event; T10-N jobId, Command responses, startAction/endAction; T11-N unitId and a successful check event.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-024 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: 01-requirements/common.md:88, 97, 104, 113, 120, 127, 134 / fixture-contract.json demoSeed.capabilities (both support temperature and have the same mode options) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added a 21-row common AT-X01–X07 N/E/B table and fixtures AT-X02-B, AT-X04-E.5, and AT-X06-B.1–.3/.5. Added tenantId to capabilities and set cap-split-std-tb on unit-tenant-b. DEC-59 covers old Context, invalid Membership periods, ineligible contracts, and identical pathLabel.
- Contract: [IR101](../../../02-design/review-resolution-contracts.md#ir101-concrete-shared-acceptance-values-and-model-registry-tenants--g1-025), DEC-59 (PROPOSED, reversible)
- Acceptance: AT-G120-025 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: verification.md:104 / strict-review-contracts.md:24, 88 / fixture-contract.json actors (tech-external-b scopes=[unit-other-customer]) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: jobs.assign/members.eligible require scope covering Job.unitId; otherwise FORBIDDEN with errors.technician_out_of_scope. Added S08 customer-b, unit-other-customer, contractor-b, and tech-external-b.
- Contract: [IR94](../../../02-design/review-resolution-contracts.md#ir94-authorization-column-qualifiers-and-technician-write-conditions--g1-001--g1-026--g1-030), DEC-54 (PROPOSED, reversible)
- Acceptance: AT-G120-026 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements client.md:214 (AT-C08-SRC) / Requirements technician.md:188 (AT-T07-SRC) / Requirements admin.md:148 (AT-A05-SRC) / fixture-contract.json demoSeed.alerts (only one window_open record) / service-contracts.ts:142 (no DemoTrigger creates inferred/inspection Alerts) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Added shared:load-cause-alerts with alert-window-a, alert-insulation-a, and alert-unknown-a, and keys AT-C08-SRC, AT-T07-SRC, and AT-A05-SRC. DemoTrigger load_alert also works at runtime.
- Contract: [IR98](../../../02-design/review-resolution-contracts.md#ir98-demo-data-for-allergen-observations-and-possible-cause-alerts--g1-006-and-g1-027), DEC-57 (PROPOSED, reversible)
- Acceptance: AT-G120-027 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: 02-design/common.md:58 / component-contracts.csv:59 (AppShell event: switchLocale;signOut;openNotifications;toggleVoice;extendSession) / screen-catalog.csv (no Screen includes demoSession.switchMembership) / 01-requirements/common.md:21 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AppShell emits switchMembership(demoMembershipId); ShellContainer calls demoSession.switchMembership.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-028 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements admin.md:250 (AT-A09-E③“No notice sent”) / review-resolution-contracts.md:36 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: AT-A09-E③: scheduling with zero customer Memberships as notice recipients returns VALIDATION under IR05 and creates zero Restrictions.
- Contract: [IR102](../../../02-design/review-resolution-contracts.md#ir102-minor-clarifications--g1-010g1-012g1-014g1-024g1-026g1-029)
- Acceptance: AT-G120-029 ([acceptance plan](../../acceptance-review-020.csv))
- Status: resolved
...
- Location: Requirements contractor.md:184 (AT-P06-N“Two technicians”) / query-catalog.csv members.capacity/members.list (filters: organizationId,qualification,activeOnly) / Design contractor.md:206-221 / fixture-contract.json actors (contractor-a has role=contractor in org-contractor-a) (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Answer: members.eligible/members.capacity and contractor members.list include technician Memberships only, excluding contractor-a itself. HQ members.list includes all roles.
- Contract: [IR94](../../../02-design/review-resolution-contracts.md#ir94-authorization-column-qualifiers-and-technician-write-conditions--g1-001--g1-026--g1-030), DEC-54 (PROPOSED, reversible)
- Acceptance: AT-G120-030 ([acceptance plan](../../acceptance-review-020.csv))
- Status: answered
...
- Location: review-resolution-contracts.md:308, 595 / Requirements technician.md:119-121 (AT-T03-N/E/B) / Requirements client.md:120, 202-204 (AT-C03-N, AT-C07-N/B) / verification.md:40 (line numbers for DOC-0.19.0, commit 0505446)
...
- Applied Revision: Answer: acceptance defaults to simulator=false. Only AT-REV18-001, AT-REV19-003, and AT-REV19-009 set it to true.
- Contract: [IR97](../../../02-design/review-resolution-contracts.md#ir97-acceptance-fixture-invariants-and-input-objects--g1-004g1-005g1-009g1-011g1-019g1-031), DEC-58 (PROPOSED, reversible)
- Acceptance: AT-G120-031 ([acceptance plan](../../acceptance-review-020.csv))
- Status: answered
...
- AT-X01–X07 lacked concrete values (G1-025). Added a common acceptance table.
- Business notification rules were missing (G1-002). Added IR95 and acceptance checks AT-P03-N④ and AT-G120-002.

### B. Design Without Requirement

- Old DD-C05 and BR-C05 required general-use consent (G1-022). Removed it and kept location consent only.
- Old BR-A14 required a factor unit (G1-020). Removed the input requirement because the unit is fixed.
...
- SCR-C08 alertCount (G1-014): added summaries.get to screens, components, and the DD catalog.
- Role switching (G1-028): defined the path through the AppShell event and ShellContainer.
- SCR-P03 jobId (G1-017): added it to url_selection.

### E. Data Model Gap

- Added releaseIntent (G1-013), Notification.templateKey values job_update/device_operation (G1-002), DemoTrigger values allergen/load_alert (G1-006/027), demoSeed.allergenObservations (G1-006), and capabilities tenantId (G1-025).

### F. Terminology Conflict

- Notification/preview (G1-009): IR97 §6 distinguishes stored inApp simulated notifications from unstored notifications.preview.
- Offline pending (G1-015): replaced it with canonical releaseState values requested/failed.

## 8. Missing Requirements

Added rules: IR94 write conditions, candidate scope, and roster roles; IR95 business notifications; IR96 cancellation; IR97 fixture invariants; IR98 allergen observations and cause alerts; IR99 guidance; IR100 components and submission; IR101 concrete acceptance values and DEC-59. No new missing requirements were found at the time of this report.

## 9. Edge Cases Not Defined

Defined edge cases: internal technician writes without an assignment; offline firmware update requests; assigning an out-of-scope technician; repeated cancellation in release_requested; cancellation in released; co2=1000 and pm25=35; both readings missing; no Unit for allergens; missing/extra components; old-scope Context; invalid Membership period; restrictionEligible=false; identical pathLabel; saving exactly at work-window end; and offline release after 30 seconds. Undefined items outside scope are in §11.

## 10. Traceability Matrix

The results for all 64 requirements are in [traceability-matrix.csv](traceability-matrix.csv) (columns: Requirement ID | Requirement | Prepare | Detailed Design | UI/UX | API | Error Handling | Testable | Before correction | After correction | G1 ID). After correction: 64 OK, 0 INCOMPLETE, 0 MISSING, 0 CONFLICT.

## 11. Undefined Decisions

| ID | Decision Needed | Related Document | Why Needed | Who Should Decide |
|---|---|---|---|---|
| DEC-54 | Technician write conditions, candidate scope, and roster scope (proposal adopted, reversible) | IR94 | Adopted to make 1A authorization deterministic; company review of business rules is needed | Security / Product Owner |
| DEC-55 | Business event notification recipients and types (proposal adopted, reversible) | IR95 | Make notification counts and templates deterministic | Product Owner / UI/UX |
| DEC-56 | Restriction cancellation state table (proposal adopted, reversible) | IR96 | Make device state handling after cancellation deterministic | Business / Security |
| DEC-57 | Allergen observation source and air quality demo thresholds (proposal adopted, reversible) | IR98 / IR99 | Make display conditions deterministic; these are not health standards | Product Owner / IoT |
| DEC-58 | Inspection component sets, submission validation, and acceptance simulator default (proposal adopted, reversible) | IR97 / IR100 | Define successful submission and reproducible tests | Product Owner / QA |
| DEC-59 | Old Context, invalid Membership period, ineligible contracts, and candidates with identical names (proposal adopted, reversible) | IR101 | Make common acceptance expectations deterministic | Security / UI/UX |
| OPEN-03–06/11 | Production API, authentication, DB, and IoT connections (existing) | PrepareDocument §10, D11 | Required before 1B; not counted as 1A blockers | Backend / IoT / Security |

## 12. Implementation Readiness

| Aspect | Judgment |
|---|---|
| Requirements | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| Detailed Design | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| UI/UX | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| API (1A local service contracts) | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| Error Handling | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| Security (1A display and action controls) | CONDITIONALLY READY (awaiting independent G1 reassessment; DEC-54/59 await company review) |
| IoT (1A demo events) | CONDITIONALLY READY (awaiting independent G1 reassessment) |
| Test / Acceptance | CONDITIONALLY READY (awaiting independent G1 reassessment; app tests not_run) |
| Agentic SDLC handoff | CONDITIONALLY READY (baseline and both validators passed; gate-G1 pending) |
| Production connection | NOT READY (D11, outside 1A scope) |

## 13. Required Actions Before Implementation

1. Have another agent run independent G1 for DOC-0.20.0 and confirm zero BLOCKER, CRITICAL, and MAJOR findings. Self-review does not replace G1.
2. Fix any G1 findings and reassess the new baseline.
3. Before company acceptance, have the owners (Product Owner / Business / Security / UI/UX / IoT / QA) review PROPOSED decisions DEC-42–59.
4. Resolve the open D11 decisions before starting 1B (production connections).
