...
| FR-T02 | P0 | 設計補完（企業目的に対応） / BIZ-06, BIZ-07, BIZ-10 | 設備台帳 | 場所、メーカー、型番、構成、設置日、保守範囲を確認。 |
...
|---|---|---|
| AT-T01-N | 社内は担当範囲、外部は自社かつ個別割当・作業期間を取得できる。 今日/期間の担当案件を開く → 異常重要度・期限・進捗で並替え → 設備詳細または作業画面へ進む。 | 参照のみ。予定0件の際は空状態と利用可能な履歴導線。 未対応はrequested全件ではなく、自分が担当して未着手の案件。社内の横断閲覧も所属テナントと担当範囲を超えない。 |
| AT-T01-E | 外部担当画面でnow=validUntilにする／別技術者のjobIdから開始する | 外部担当の期限終了をまたぐと当該live設備を隠す。別技術者の案件ID直打ちでも作業開始不可。 |
| AT-T01-B | 自分のassignedと他人のrequested、社内scope内外・外部割当期間内外を表示する | 未対応はrequested全件ではなく、自分が担当して未着手の案件。社内の横断閲覧も所属テナントと担当範囲を超えない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T02-N | 対象設備に閲覧可能な担当関係がある。 案件から台帳を開く → 設置場所・型番・構成・設置日・保守範囲を確認 → 診断/作業へ進む。 | 参照のみ。技術者はメーカー台帳・顧客所属・請求を変更しない。 機種能力はcapability版を表示。未登録/不明項目は未登録と表示し、典型機種の値で補完しない。 |
| AT-T02-E | installedAt=nullの台帳を開く／外部技術者で非割当unitIdを開く | 台帳に設置日なしなら現在日を補わない。外部技術者が非割当設備を参照しても情報を返さない。 |
| AT-T02-B | 能力登録済みと未登録の設備を開く | 機種能力はcapability版を表示。未登録/不明項目は未登録と表示し、典型機種の値で補完しない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T03-N | 対象設備のtelemetry閲覧権。センサーなしでも通信情報は表示可能。 指標と期間を選ぶ → センサー/電力/運転/通信の値と時刻を確認 → デモ更新イベントで変更 → 切断時は更新停止を認識する。 | 最新値と時系列を同じeventId/版で整合させる。画面離脱・scope変更で購読解除。 観測時刻と受信時刻を分け、staleAfterSeconds超過はstale。古いイベントを最新値へ上書きしない。系列の単位を固定し欠測の間を連結しない。 |
| AT-T03-E | event version=3の後に2と重複3を送る／通信断にする | 順序逆転・重複イベントで値が逆行しない。通信断時も最後の値は時刻付きで残せるがリアルタイムと表示しない。 |
| AT-T03-B | 観測から120秒と120秒+1ms、重複・古いイベント、系列の途中nullを表示する | 観測時刻と受信時刻を分け、staleAfterSeconds超過はstale。古いイベントを最新値へ上書きしない。系列の単位を固定し欠測の間を連結しない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T04-N | 有効な担当案件がin_progressである。保守範囲と部品別の点検対象が取得済み。 対象部品ごとに点検結果を選ぶ → 必要な所見・測定・写真を関連付け → 未点検/対象外は理由を記入 → ドラフトまたは報告へ保存する。 | 点検結果を報告版・作者・観測時刻へ紐付ける。センサー推定は別の根拠として残し、現地点検結果で元データを上書きしない。 対象グループはindoor、項目はfilter / evaporator_coil / blower_motor / blower_fan / drain_pipe / drain_pan / outlet / louver。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
| AT-T04-E | 提出時に結果null・測定単位なし・not_inspected理由なし・他案件attachmentIdを個別に設定する | 未入力、単位なし測定、未点検理由なし、他案件の写真参照を拒否。正常を初期選択して空点検を完了扱いしない。 |
| AT-T04-B | indoor全8部品をnormal/attention/not_inspected/not_applicable/nullで個別に保存・提出する | 対象グループはindoor、項目はfilter / evaporator_coil / blower_motor / blower_fan / drain_pipe / drain_pan / outlet / louver。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T05-N | 有効な担当案件がin_progressである。保守範囲と部品別の点検対象が取得済み。 対象部品ごとに点検結果を選ぶ → 必要な所見・測定・写真を関連付け → 未点検/対象外は理由を記入 → ドラフトまたは報告へ保存する。 | 点検結果を報告版・作者・観測時刻へ紐付ける。センサー推定は別の根拠として残し、現地点検結果で元データを上書きしない。 対象グループはoutdoor、項目はcondenser_coil / compressor / fan / blade / refrigerant_pipe。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
| AT-T05-E | 提出時に結果null・測定単位なし・not_inspected理由なし・他案件attachmentIdを個別に設定する | 未入力、単位なし測定、未点検理由なし、他案件の写真参照を拒否。正常を初期選択して空点検を完了扱いしない。 |
| AT-T05-B | outdoor全5部品をnormal/attention/not_inspected/not_applicable/nullで個別に保存・提出する | 対象グループはoutdoor、項目はcondenser_coil / compressor / fan / blade / refrigerant_pipe。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T06-N | 有効な担当案件がin_progressである。保守範囲と部品別の点検対象が取得済み。 対象部品ごとに点検結果を選ぶ → 必要な所見・測定・写真を関連付け → 未点検/対象外は理由を記入 → ドラフトまたは報告へ保存する。 | 点検結果を報告版・作者・観測時刻へ紐付ける。センサー推定は別の根拠として残し、現地点検結果で元データを上書きしない。 対象グループはelectrical、項目はthermostat / sensor / capacitor / contactor / wiring。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
| AT-T06-E | 提出時に結果null・測定単位なし・not_inspected理由なし・他案件attachmentIdを個別に設定する | 未入力、単位なし測定、未点検理由なし、他案件の写真参照を拒否。正常を初期選択して空点検を完了扱いしない。 |
| AT-T06-B | electrical全5部品をnormal/attention/not_inspected/not_applicable/nullで個別に保存・提出する | 対象グループはelectrical、項目はthermostat / sensor / capacitor / contactor / wiring。初期結果はnullで、未点検のまま提出するなら明示的なnot_inspectedと理由が必要。設備に存在しない部品はnot_applicableと理由を記録する。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T07-N | 担当設備に異常または診断の疑いがある。 異常一覧から根拠を開く → 計測/推定/現地点検と履歴を確認 → 確認済みにする → 必要なら再測定・理由付き解消へ進む。 | 検知、確認、解消の各時刻と主体を保持。再発は新alertIdで旧事象と関連付ける。 推定に確信度がない場合は数値の確率を生成しない。確認操作はacknowledged。解消には再測定で設定条件を満たすかalert.resolve権限と理由が必要。 |
| AT-T07-E | 未解消AlertのJobだけcompletedにする／heartbeat途絶だけを与える | Job完了だけではresolvedにしない。通信断だけの根拠で盗難と断定せず、取り外し専用事象と分離する。 |
| AT-T07-B | 確信度なしの推定、権限あり理由付き解消、権限なし手動解消、再測定で回復条件成立を比較する | 推定に確信度がない場合は数値の確率を生成しない。確認操作はacknowledged。解消には再測定で設定条件を満たすかalert.resolve権限と理由が必要。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T08-N | assigned案件を担当し、有効期間内。定期/事後/予防は同じ作業状態モデル。 担当と予定を確認 → 作業開始 → 報告を編集 → 提出 → 品質確認待ち → 差戻しなら再作業と再提出。 | 開始時刻・提出時刻・reportVersionを保存。完了は品質担当のreviewで決定。 開始可能はassigned、差戻し再開はrework_requested。submittedでは提出版を読取専用。顧客承認を技術者が代行しない。 |
| AT-T08-E | requested・cancelled・on_hold・担当期限外でstart/submitする／有効in_progressでsubmitを失敗させる | 未割当、取消、on_hold、期限外のstart/submitを拒否。提出失敗ではin_progressとドラフトを保持する。 |
| AT-T08-B | assignedの開始、submittedの編集、rework_requestedの再開を比較する | 開始可能はassigned、差戻し再開はrework_requested。submittedでは提出版を読取専用。顧客承認を技術者が代行しない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T09-N | in_progressまたは再作業中。点検項目とドラフトを取得済み。 点検・測定・写真・交換部品・作業本文・次回対応を入力 → ドラフト保存 → 提出前チェック → 報告版を確定する。 | 保存成功時にdraft版を更新し、提出時は固定したreportVersionをJobへ紐付ける。写真削除時にobject URLを解放する。 報告は作者・版を保持。画像はJPEG/PNG、1枚5MiB以下、最大10枚。数量は正整数、次回対応はnoneまたは日時/内容を明示。サービス再取得でdirty内容を消さない。 |
| AT-T09-E | 本文9・4001文字／偽MIME／写真11枚・5MiB+1byte／部品数量0／写真状態failedで提出する | 本文9/4001文字・部品数量0は提出を拒否。偽MIME・11枚目・5MiB超過は画像追加を拒否。画像処理失敗はfailedと表示し、失敗写真が残るまま提出できない。保存済み本文・画像は保持する。 |
| AT-T09-B | 5MiBのJPEG/PNG各10枚、部品数量1、nextAction none/follow_upを保存し、dirty中に再取得する | 報告は作者・版を保持。画像はJPEG/PNG、1枚5MiB以下、最大10枚。数量は正整数、次回対応はnoneまたは日時/内容を明示。サービス再取得でdirty内容を消さない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T10-N | 担当期間内、control.diagnose能力、機器online。契約制限を超えない。 現在状態・担当案件を確認 → 診断操作・試運転時間・理由を指定 → 確認 → Commandの応答と履歴を見る。 | 通常診断は理由とjobId付きCommand、試運転は時間・終了動作を保持するDiagnosticRunと開始/終了Commandを作成。終了予定と終了応答を別表示し、終了失敗は注意として残す。 同時FW更新や未完了Command中は開始不可。試運転終了も終了Commandの応答が必要で、ブラウザタイマー終了を実停止扱いしない。 |
| AT-T10-E | 制限下限未満の温度／担当期間外／durationMinutes=16／空理由で開始する／終了応答を与えない | 制限温度の迂回、担当期間外、16分の試運転、理由なしを拒否。終了応答がなければ停止済みにしない。 |
| AT-T10-B | 未完了Commandあり/FW更新中の開始、終了時計到来と終了応答到来を比較する | 同時FW更新や未完了Command中は開始不可。試運転終了も終了Commandの応答が必要で、ブラウザタイマー終了を実停止扱いしない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T11-N | device.maintainと対象設備の有効担当。登録/校正/更新はモック。 serial登録 → 設備紐付け → 接続確認 → 校正値と参照を記録 → 対応FW候補を選び更新 → 進行/結果を確認する。 | Device、CalibrationRecord、DeviceOperationを保持。succeededだけfirmwareVersion更新。更新中は競合制御不可。 serialはtrim/大文字化して一意判定。校正は履歴追加、既存測定値は書き換えない。FWは対応版リストからのみ選択。URLやバイナリを自由入力させない。 |
| AT-T11-E | 正規化後に同じserial／他設備への理由なし再紐付け／offline更新／校正単位不一致／FW失敗を個別に与える | 重複serial・別設備への無断再紐付け・単位不一致を拒否し台帳を変更しない。offlineではFW更新を開始しない。FW更新失敗はfailedと表示し、旧FW版を保持する。 |
| AT-T11-B | serial小文字と大文字、校正前後の測定履歴、対応/非対応FWと成功/失敗を比較する | serialはtrim/大文字化して一意判定。校正は履歴追加、既存測定値は書き換えない。FWは対応版リストからのみ選択。URLやバイナリを自由入力させない。 |
...
| 受入ID | Given / When | Then（観測可能な結果） |
|---|---|---|
| AT-T12-N | 担当Deviceにイベント閲覧権がある。 通信断・電源断・取り外し検知を別々に模擬発火 → 通知を確認 → 対応メモ → 復旧/確認を記録する。 | 検知時刻・観測根拠・対応・復旧時刻を別イベントで保持。通知の確認は物理状態を変更しない。 connectionとtamperを独立管理。電源断判定は電源専用信号のデモがある時のみ。heartbeatなしから電源断と断定しない。 |
| AT-T12-E | tamper未確認のまま通信復旧させる／新しい通信断の後に古いheartbeatを与える | 通信再接続で未確認のtamperアラートが消えない。out-of-orderの古いheartbeatでonlineへ戻さない。 |
| AT-T12-B | heartbeat途絶・power_signal断・tamper_signal発火をそれぞれ与える | connectionとtamperを独立管理。電源断判定は電源専用信号のデモがある時のみ。heartbeatなしから電源断と断定しない。 |
