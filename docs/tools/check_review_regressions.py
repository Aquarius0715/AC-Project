...
CASES = [
    ...
    ('stale_handoff_baseline', [('04-agentic-sdlc/README.md','runs/DOC-0.21.0/spec-manifest.json','runs/DOC-0.20.0/spec-manifest.json')], 'Current handoff baseline mismatch'),
    ...
    ('notification_alert_exception_removed', [('02-design/review-resolution-contracts.md', 'templateKey=alertはIR10のAlert分類に従い、それ以外はtemplateKeyとtypeを同名にする', 'templateKeyとtypeは同名（IR10）')], '0.21 behavioral guard missing (IR95)'),
    ...
    ('a12_start_counter_backdated', [('02-design/review-resolution-contracts.md', '保存前の履歴や遅着FactのobservedAtを開始時刻にしない', '保存前の履歴を開始時刻として使う')], '0.21 behavioral guard missing (IR103)'),
    ...
    ('allergen_replay_dedup_removed', [('02-design/review-resolution-contracts.md', '同じDemoTrigger.eventIdの再送は行と変更イベントを増やさない', '再送でも行と変更イベントを増やす')], '0.21 behavioral guard missing (IR105)'),
    ('notification_scope_default_rule_removed', [('02-design/review-resolution-contracts.md', 'scopeVersionAtCreationが省略されている場合、全patch適用後のrecipientMembershipIdで引いたMembership.scopeVersionを設定する', 'scopeVersionAtCreationは省略のままにする')], '0.21 behavioral guard missing (IR106)'),
